...
void LLVMCodeGen::cleanupCurrentScope() {
    ...
    for (const auto& var : scopeVars) {
        // Only free dynamic heap arrays, avoiding raw string literals or static memory
...
void LLVMCodeGen::visit(StringLiteralASTNode* node) {
    ...
    // We split on ${...} boundaries and concatenate
    ...
    // The raw content has ${...} placeholders
    // Build segments: [literal, expr-text, literal, ...] interleaved
    // We only build the literal parts here (expr parts were not separately tokenized,
    // so for now we fallback to emitting the raw string with ${...} markers intact)
    // A proper implementation requires a separate pass during parsing to build
    // InterpolatedString AST nodes. This implementation emits literal segments
    // as the content (future: parser will split into segments).

    // For now: strip the ${...} markers and treat as plain string with a note
    // Real interpolation: split on "${" and "}" and emit concat chain
    std::string result;
    std::string buf;

    // Allocate a large buffer for the template result
    ...
    while (pos < strVal.size()) {
        ...
        if (dollarPos != std::string::npos) {
            ...
            // Compile the expression: tokenize and parse just the expression
            try {
                Lexer exprLexer(exprText + ";");
                Parser exprParser(std::move(exprLexer));
                // Parse as expression
                // We'll wrap in a dummy function to get the expression
                ...
                // If expression parse fails, emit the raw text
            }
        }
    }
...
void LLVMCodeGen::visit(BinaryOpASTNode* node) {
    ...
    if (op == "+") {
        ...
        // Bitwise AND — convert to int first
